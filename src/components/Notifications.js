...
import { acceptContact, rejectContact, declined_room } from "../services/sockets/sockets";
...
function Notifications() {
    ...
    return (
        <div className="notifications">
            <div onClick={() => setBell(bellState ? false : true)} className="bell-icon">
...
function BellComponent({ data, request, opts }) {
    ...
    const { bellState } = useContext(roomsContext);
                    ...
                    <div className="bell-content">
                        ...
                        {opts ? (
                            <div className="bell-options">
                                <button onClick={() => acceptContact({ id: _id, token, img: user.img })} className="accept button">Accept</button>
                                <button onClick={() => data.room_id? declined_room({id: _id, img: user.img}) : rejectContact({ id: _id, token, img: user.img }) } className="reject button">Reject</button>
                            </div>
                        ) : ""}
